...
const readFixedDecimals = (seg, n, { allowDash = false, decimals = 2 } = {}) => {
  ...
  const d = `{${decimals}}`;
  const re = allowDash
    ? new RegExp(`(?:\\d+\\.\\d${d}|\\.\\d${d}|[-ー])`, "g")
    : new RegExp(`(?:\\d+\\.\\d${d}|\\.\\d${d})`, "g");
  const vals = [];
  let m;
...
export default function App() {
  ...
  const parseTenji = () => {
    ...
    try {
      ...
      readRow("ST", { allowDash: true, fixedDecimal: 2 });   // 無視（平均ST欄を上書きしないため）
    ...
    try {
      ...
      for (const blk of blocks) {
        ...
        for (let i = 0; i + 2 < r0.length + 1; i += 3) {
          const sec = Number(r0[i]), third = Number(r0[i + 1]), o = Number(r0[i + 2]);
          if ([sec, third].every((x) => x >= 1 && x <= 6) && o > 0) {
            colSeconds.push(sec);
            if (third !== first && third !== sec) result[`${first}-${sec}-${third}`] = o;
          }
        }
      ...
      if (Object.keys(result).length < 10) {
        setPMsg("odds", "オッズを十分に読み取れませんでした。オッズ一覧を選手名の行ごと丸ごとコピーしてください");
        return;
      }
      setOdds(result);
